...
def deck(deck:str):
    ...
    arr[-1].append(s)
    order="AKQJT987654321"
    arr[0].sort(key=lambda card : order.index(card[0]))
    arr[1].sort(key=lambda card : order.index(card[0]))
    return arr
#print(deck(arr[0]))

def hand(deck:list[list[str]]):
    order="AKQJT987654321"
    hands=[[[],[],[],[]],[[],[],[],[]]]
    for i in deck[0]:
        if(i[1]=="D"):
    ...
    for i in deck[1]:
        if(i[1]=="D"):
            ...
            return ValueError
    return hands
#print(hand(deck(arr[0])))

def royal_flush(arr:list[list[list[str]]]):
    b=0
    for i in arr[0]:
        if(len(i)>0 and len(i)<5):
            break
        elif(len(i)==5):
            if(i[0][0]=="A" and i[5][0]=="T"):
                b+=1
    for i in arr[1]:
        if(len(i)>0 and len(i)<5):
            break
    ...
    if(b):
        ...
        return [False]
#print(royal_flush(hand(deck(arr[0]))))

def straight_flush(arr:list[list[list[str]]]):
    b=0
    c=""
    order="AKQJT987654321"
    for i in arr[0]:
        if(len(i)>0 and len(i)<5):
            ...
            r=0
            while(r<4):
                if(order.index(i[r][0])-order.index(i[r+1][0])!=1):
                    b-=1
                r+=1
            b+=1
            c=i[0][0]
        ...
        if(len(i)>0 and len(i)<5):
            ...
            while(r<4):
                if(order.index(i[r][0])-order.index(i[r+1][0])!=1):
                    b-=2
                r+=1
            if(b and order.index(c)>order.index(i[1][0])):
                b=0
            elif(b and order.index(c)==order.index(i[1][0])):
                b+=2
            else:
                b+=2
    if(b):
        ...
        return [False]
#print(straight_flush(hand(deck(arr[0]))))   

def four_of_a_kind(arr:list[list[str]]):
    b=0
    c=""
    d=""
    if(arr[0][0][0] == arr[0][3][0] or arr[0][1][0] == arr[0][4][0]):
        b+=1
        c=arr[0][2][0]
        if(arr[0][0][0]==arr[0][2][0]):
            d=arr[0][4][0]
        else:
            d=arr[0][0][0]
    if(arr[1][0][0] == arr[1][3][0] or arr[1][1][0] == arr[1][4][0]):
        tem=""
        if(arr[1][0][0]==arr[1][2][0]):
            tem=arr[1][4][0]
        else:
            tem=arr[1][0][0]
        if(b and order.index(c)>order.index(arr[1][2][0])):
            b+=2
        elif(b and order.index(c)<order.index(arr[1][2][0])):
            b+=0
        elif(b and order.index(d)>order.index(tem)):
            b+=2
        elif(b and order.index(d)<order.index(tem)):
            b+=0
        else:
    ...
    if(b):
        ...
        return [False]
#print(four_of_a_kind(deck(arr[0])))  

def full_house(arr:list[list[str]]):
    b=0
    c=""
    d=""
    if(arr[0][2][0]==arr[0][0][0]):
        if(arr[0][3][0]==arr[0][4][0]):
            b+=1
            c=arr[0][2][0]
            d=arr[0][4][0]
    elif(arr[0][2][0]==arr[0][4][0]):
        if(arr[0][0][0]==arr[0][1][0]):
            b+=1
            c=arr[0][2][0]
            d=arr[0][0][0]
    if(arr[1][2][0]==arr[1][0][0]):
        if(arr[1][3][0]==arr[1][4][0]):
            if(b):
                if(order.index(c)>order.index(arr[1][2][0])):
                    b=2
                elif(order.index(c)<order.index(arr[1][2][0])):
                    b+=0
                elif(order.index(d)>order.index(arr[1][4][0])):
                    b=2
                elif(order.index(d)>order.index(arr[1][4][0])):
                    b+=0
                else:
        ...
        if(arr[1][0][0]==arr[1][1][0]):
            if(b):
                if(order.index(c)>order.index(arr[1][2][0])):
                    ...
                    b+=0
                elif(order.index(d)>order.index(arr[1][0][0])):
                    b=2
                elif(order.index(d)>order.index(arr[1][0][0])):
                    b+=0
                else:
    ...
    if(b):
        ...
        return [False]
#print(full_house(hand(deck(arr[0]))))

def flush(arr:list[list[list[str]]]):
    b=0
    for i in arr[0]:
        if(len(i)>0 and len(i)<5):
            b-=1
    ...
    if(b):
        ...
        return [False]
#print(flush(hand(deck(arr[0]))))

def straight(arr:list[list[str]]):
    b=0
    c=""
    order="AKQJT987654321"
    for r in range(4):
        if(order.index(arr[0][r+1][0])-order.index(arr[0][r][0])!=1):
            b-=1
            break
    b+=1
    if(b):
    ...
    r=0
    for r in range(4):
        if(order.index(arr[1][r+1][0])-order.index(arr[1][r][0])!=1):
            b-=2
            break
    if(b==1):
        if(order.index(c)>order.index(arr[1][0][0])):
            b=2
        elif(order.index(c)<order.index(arr[1][0][0])):
            b+=0
        else:
            b+=2
    else:
    ...
    if(b):
        ...
        return [False]
#print(straight(deck(arr[0])))

def three_of_a_kind(arr:list[list[str]]):
    b=0
    if(arr[0][0][0] == arr[0][2][0] or arr[0][1][0] == arr[0][3][0] or arr[0][2][0] == arr[0][4][0]):
        b+=1
    if(arr[1][0][0] == arr[1][3][0] or arr[1][1][0] == arr[1][4][0] or arr[1][2][0] == arr[1][4][0]):
        b+=2
    if(b):
        return [True,b]
    else:
        return [False]
#print(three_of_a_kind(deck(arr[0])))

def two_pairs(arr:list[list[str]]):
    b=0
    s1,s2="",""
    for i in range(4):
    ...
    if(c<4):
        c=0
    if(b+c==6):
        s11,s12,s21,s22=order.index(s1[0]),order.index(s1[1]),order.index(s2[0]),order.index(s2[1])
        if(max(s11,s12)>max(s21,s22)):
            return [True,2]
        elif(max(s11,s12)<max(s21,s22)):
            return [True,1]
        elif(min(s11,s12)>min(s21,s22)):
            return [True,2]
        elif(min(s11,s12)<min(s21,s22)):
            return [True,1]
        else:
        ...
        return [False]
#print(two_pairs(deck(arr[0])))

def one_pair(arr:list[list[str]]):
    b=0
    c=""
    for i in range(4):
        if(arr[0][i][0]==arr[0][i+1][0]):
    ...
    for i in range(4):
        if(arr[1][i][0]==arr[1][i+1][0]):
            if(b):
                if(order.index(c)>order.index(arr[1][i][0])):
                    b=2
                elif(order.index(c)<order.index(arr[1][i][0])):
                    b+=0
                else:
                    b+=2
            else:
    ...
    if(b):
        ...
        return [False]
#print(one_pair(deck(arr[0])))

def high_card(arr:list[list[str]]):
    order="AKQJT987654321"
    for i in range(5):
        s=order.index(arr[0][i][0])-order.index(arr[1][i][0])
        if(s>0):
            return [True,2]
        elif(s<0):
            return [True,1]
#print(high_card(deck(arr[0])))
...
def Poker(cards :str):
    ...
    r=royal_flush(h)
    if(r[0]):
        print("Royal Flush:",r[1])
        return r[1]
    r=straight_flush(h)
    if(r[0]):
        print("Straight Flush:",r[1])
        return r[1]
    r=four_of_a_kind(d)
    if(r[0]):
        print("Four of a kind:",r[1])
        return r[1]
    r=full_house(d)
    if(r[0]):
        print("Full House:",r[1])
        return r[1]
    r=flush(h)
    if(r[0]):
        if(r[1]==3):
            r=high_card(d)
            print("Flush and Highest Card:",r[1])
        else:
            print("Flush:",r[1])
        return r[1]
    r=straight(d)
    ...
    if(r[0]):
        if(r[1]==3):
            r=high_card(d)
            print("Three of a kind and High Card:",r[1])
        else:
            print("Three of a kind:",r[1])
        return r[1]
    r=two_pairs(d)
    if(r[0]):
        if(r[1]==3):
            r=high_card(d)
            print("Two pairs and High Card:",r[1])
        else:
            print("Two pairs:",r[1])
        return r[1]
    r=one_pair(d)
    if(r[0]):
        if(r[1]==3):
            r=high_card(d)
            print("One pairs with high card:",r[1])
        else:
            print("One pairs:",r[1])
        return r[1]
    r=high_card(d)
...
def Poker_Game(arr:list[str]):
    counter=0
    for i in arr:
        w=Poker(i)
        if(w==1):
            counter+=1
        print(arr.index(i),counter)
    return counter
print(Poker_Game(arr))